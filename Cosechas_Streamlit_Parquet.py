...
def load_excel(uploaded_excel) -> pd.DataFrame:
    df = pd.read_excel(uploaded_excel, sheet_name="Data")

    # MUY IMPORTANTE (formato colombiano)
...
def prepare_data(df_raw: pd.DataFrame):
    ...
    # Fechas (por si vienen como object)
...
def render_heatmap_fast(vint_pct: pd.DataFrame, title: str, force_values: bool):
    ...
    # ✅ Ajuste visual: más peso a valores altos (más rojo)
...
with st.sidebar:
    ...
    st.subheader("🔄 Convertir Excel → Parquet (opcional)")
    excel_upload = st.file_uploader("Sube data_blog.xlsx", type=["xlsx"], key="excel_upload")

    parquet_bytes = None
    if excel_upload is not None:
        if st.button("⚙️ Convertir a Parquet", use_container_width=True):
            try:
                with st.spinner("Convirtiendo Excel a Parquet..."):
                    df_excel = load_excel(excel_upload)

                    parquet_buffer = io.BytesIO()
                    df_excel.to_parquet(parquet_buffer, index=False)
                    parquet_buffer.seek(0)
                    parquet_bytes = parquet_buffer.getvalue()

                st.success("✅ Parquet generado")
            except Exception as e:
                st.error(f"Error convirtiendo Excel: {e}")

    if parquet_bytes is not None:
        st.download_button(
            label="⬇ Descargar data_blog.parquet",
            data=parquet_bytes,
            file_name="data_blog.parquet",
            mime="application/octet-stream",
            use_container_width=True
        )

    st.divider()
    st.subheader("📦 Cargar Parquet para el Dashboard")
    uploaded = st.file_uploader("Sube data_blog.parquet", type=["parquet"], key="parquet_upload")

if uploaded is None:
    st.info("Sube el archivo **.parquet** para empezar (puedes convertir Excel arriba).")
    st.stop()
...
if not st.session_state.computed:
    st.info("Configura filtros y presiona **Aplicar filtros** una vez. Luego líneas será automático.")
    st.stop()
